...
public class SwipeRotationsProblem : MonoBehaviour {
    ...
    void Update()
    {
        ...
        else if (Input.GetMouseButtonUp(0))
        {
            ...
            if (movementRatio > thresholdRatio && length > thresholdLength)
            {
                ...
                if (start.x > end.x)
                {
                    GameObject.Find("UIController").GetComponent<UIHandler_ProbScene>().CameraRotate(true);
                }
                else
                {
                    GameObject.Find("UIController").GetComponent<UIHandler_ProbScene>().CameraRotate(false);
                }
            }
            else
            {
